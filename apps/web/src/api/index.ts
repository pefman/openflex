...
export const moviesApi = {
  ...
  removeFiles: (id: number) => api.delete<{ removedFiles: number }>(`/api/movies/${id}/files`).then(r => r.data),
  removeFile: (id: number, fileId: number) => api.delete<{ success: boolean; remainingFiles: number }>(`/api/movies/${id}/files/${fileId}`).then(r => r.data),
  bulkUpdate: (ids: number[], data: { monitored?: boolean; status?: string }) => api.patch<{ updated: number }>('/api/movies/bulk', { ids, data }).then(r => r.data),
...
export interface ImdbListSubscriptionDto {
  id: number
  userId: number
  source: string
  externalListId: string
...
export const watchlistApi = {
  ...
  importImdbList: (url: string) =>
    api.post<ImdbImportSummary>('/api/watchlist/imdb/import', { url }).then(r => r.data),
  listImdbSubscriptions: () =>
    api.get<ImdbListSubscriptionDto[]>('/api/watchlist/imdb/subscriptions').then(r => r.data),
  createImdbSubscription: (body: { url: string; enabled?: boolean; syncIntervalHours?: number }) =>
    api.post<{ subscription: ImdbListSubscriptionDto; initialSync: unknown }>('/api/watchlist/imdb/subscriptions', body).then(r => r.data),
  updateImdbSubscription: (id: number, body: { enabled?: boolean; syncIntervalHours?: number }) =>
    api.patch<ImdbListSubscriptionDto>(`/api/watchlist/imdb/subscriptions/${id}`, body).then(r => r.data),
  removeImdbSubscription: (id: number) =>
    api.delete<{ success: boolean }>(`/api/watchlist/imdb/subscriptions/${id}`).then(r => r.data),
  syncImdbSubscription: (id: number) =>
    api.post<{ skipped: boolean; reason?: string; summary?: ImdbImportSummary }>(`/api/watchlist/imdb/subscriptions/${id}/sync`).then(r => r.data),
}
