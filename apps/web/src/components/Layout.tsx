...
import { downloadsApi, logsApi, optimizationApi } from '../api/index.ts'
...
const RUNNING_DOWNLOAD_STATUSES = new Set(['downloading', 'importing', 'verifying'])

function useDownloadQueueCounts() {
  const { data = [] } = useQuery({
    queryKey: ['downloads'],
  ...
  const running = data.filter((d) => RUNNING_DOWNLOAD_STATUSES.has(d.status)).length
  const queued = data.filter((d) => d.status === 'queued').length
  return { running, queued, total: running + queued }
}

function useOptimizationQueueCounts() {
  const { data: runningJobs = [] } = useQuery({
    queryKey: ['optimization-jobs', 'running-count'],
    queryFn: () => optimizationApi.listJobs({ status: 'running', limit: 500 }),
    refetchInterval: 5000,
    staleTime: 3000,
  })

  const { data: queuedJobs = [] } = useQuery({
    queryKey: ['optimization-jobs', 'queued-count'],
    queryFn: () => optimizationApi.listJobs({ status: 'queued', limit: 500 }),
    refetchInterval: 5000,
    staleTime: 3000,
  })
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const downloadCounts = useDownloadQueueCounts()
  const optimizationCounts = useOptimizationQueueCounts()
              ...
              {group.items.map((item) => (
                ...
                >
                  ...
                  <span className="flex-1">
                    {item.to === '/downloads' && downloadCounts.total > 0
                      ? `Downloads (${downloadCounts.running}/${downloadCounts.total})`
                      : item.to === '/optimization' && optimizationCounts.total > 0
                        ? `Optimization (${optimizationCounts.running}/${optimizationCounts.total})`
                        : item.label}
                  </span>
