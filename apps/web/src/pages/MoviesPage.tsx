...
export default function MoviesPage() {
  ...
  const bulkStatusMutation = useMutation({
    mutationFn: (status: 'wanted' | 'unwanted') => moviesApi.bulkUpdate([...selectedIds], { status }),
    onSuccess: (_, status) => {
      qc.invalidateQueries({ queryKey: ['movies'] })
      setSelectedIds(new Set())
      toast.success(`Set ${selectedIds.size} movie(s) to ${status}`)
    },
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 bg-popover border rounded-xl shadow-lg px-4 py-3">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => bulkStatusMutation.mutate('wanted')}
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => bulkStatusMutation.mutate('unwanted')}
            disabled={bulkStatusMutation.isPending}
          >
            Set Unwanted
          </Button>
...
function MovieCard({
  ...
  return (
    <div className="group relative">
      <Link to={moviePath(movie.title, movie.id)}>
