...
import { moviePath, slugify, formatBytes, formatSpeed } from '@/lib/utils'
...
export default function Dashboard() {
  ...
  const downloadedMovieItems: RecentMedia[] = movies
    .filter((m) => m.mediaFiles.length > 0)
    .map((m) => ({
      id: m.id,
      title: m.title,
      posterPath: m.posterPath,
      year: m.year,
      added: m.mediaFiles.reduce((latest, f) => (f.addedAt > latest ? f.addedAt : latest), m.mediaFiles[0].addedAt),
      type: 'movie' as const,
    }))

  const downloadedShowItems = shows
    .map((s) => {
      const episodeFiles = s.seasons.flatMap((se) => se.episodes.flatMap((ep) => ep.mediaFiles))
      if (episodeFiles.length === 0) return null
      return {
        id: s.id,
        title: s.title,
        posterPath: s.posterPath,
        added: episodeFiles.reduce((latest, f) => (f.addedAt > latest ? f.addedAt : latest), episodeFiles[0].addedAt),
        type: 'show' as const,
      }
    })
  ...
  return (
    <div className="p-6 space-y-8">
      {/* Recently Downloaded (movies + shows combined) */}
      {recentMedia.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold">Recently Downloaded</h2>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-10 gap-3">
            {recentMedia.map((item) => (
              <Link
                key={`${item.type}-${item.id}`}
                to={item.type === 'movie' ? moviePath(item.title, item.id) : `/shows/${slugify(item.title)}`}
                className="group"
              >
