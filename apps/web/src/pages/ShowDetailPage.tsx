...
import { slugify, cn, formatDate, playerPath } from '@/lib/utils'
          ...
          <Accordion type="single" collapsible defaultValue={defaultSeason} className="space-y-2">
            {[...show.seasons].sort((a, b) => b.seasonNumber - a.seasonNumber).map((season) => (
              <SeasonAccordionItem
                ...
                onPlay={(mediaFileId, label) => navigate(playerPath(label, mediaFileId))}
              />
            ))}
          </Accordion>
        </div>
...
function SeasonAccordionItem({
  season, showId, showTitle, optimizationProfileId, onPlay,
}: {
  season: SeasonDto
  showId: number
  showTitle: string
  optimizationProfileId: number | null
  onPlay: (id: number, label: string) => void
}) {
  const qc = useQueryClient()
...
function EpisodeRow({ episode, showId, showTitle, seasonNumber, optimizationProfileId, onPlay }: { episode: EpisodeDto; showId: number; showTitle: string; seasonNumber: number; optimizationProfileId: number | null; onPlay: (id: number, label: string) => void }) {
  const qc = useQueryClient()
      ...
      {hasFile && (
        <>
          <Button
            ...
            onClick={() => onPlay(f.id, `${showTitle}-s${String(seasonNumber).padStart(2, '0')}e${String(episode.episodeNumber).padStart(2, '0')}`)}
          >
