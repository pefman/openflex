...
import { moviePath, slugify, formatDate } from '@/lib/utils'
            ...
            <CardContent className="p-0 divide-y divide-border">
              {wantedMovies.map((movie) => (
                <div key={movie.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={moviePath(movie.title, movie.id)} className="text-sm font-medium hover:text-primary truncate block">
                      {movie.title}
                    </Link>
