...
export const showRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.patch<{ Body: { ids: number[]; data?: { monitored?: boolean }; monitored?: boolean } }>(
    '/bulk',
    { preHandler: [requireAuth] },
    async (req, reply) => {
      const ids = req.body?.ids
      const data = req.body?.data ?? { monitored: req.body?.monitored }
      if (!Array.isArray(ids) || ids.length === 0) return reply.code(400).send({ error: 'ids required' })
      if (data.monitored == null) return reply.code(400).send({ error: 'update data required' })
