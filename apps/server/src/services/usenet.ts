  ...
  for (const segment of segments) {
    try {
      ...
      if (failedSegments <= 3) {
        ...
        suppressedSegmentLogs++
      }
      // Continue with remaining segments
    }
  }

  if (suppressedSegmentLogs > 0) {
    log('warn', 'usenet', `suppressed ${suppressedSegmentLogs} additional failed segment log(s) for ${extractFilename(file.subject) ?? 'nzb file'}`)
  }
...
async function downloadFileInParallel(
  ...
  let segIdx = 0
  let cancelled = false
  let loggedFailedSegments = 0
  let suppressedFailedSegmentLogs = 0
    ...
    const tryLaunch = () => {
      while (active < maxConns && segIdx < segments.length && !cancelled) {
        ...
        client.connect()
          ...
          .catch(async (err) => {
            if (!cancelled) {
              if (loggedFailedSegments < 3) {
                log('warn', 'usenet', `failed segment ${segment.messageId}: ${err}`)
                loggedFailedSegments++
              } else {
                suppressedFailedSegmentLogs++
              }
            }
            parts[idx] = Buffer.alloc(0)
            failedSegments++

            // Cancellation can happen while segments are already in flight.
            // Periodically check if the download still exists/is active and abort quickly.
            if (!cancelled && downloadId && failedSegments % 8 === 0) {
              const state = await db.download.findUnique({ where: { id: downloadId }, select: { status: true } }).catch(() => null)
  ...
  if (suppressedFailedSegmentLogs > 0) {
    log('warn', 'usenet', `suppressed ${suppressedFailedSegmentLogs} additional failed segment log(s) during parallel download`)
  }
