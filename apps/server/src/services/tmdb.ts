...
export async function findTmdbByImdbId(imdbId: string): Promise<{ movieTmdbId: number | null; showTmdbId: number | null }> {
  const apiKey = await getApiKey()
  const url = `${TMDB_BASE}/find/${encodeURIComponent(imdbId)}?api_key=${apiKey}&external_source=imdb_id`
  const data = await fetchJson<{ movie_results?: Array<{ id: number }>; tv_results?: Array<{ id: number }> }>(url)
  return {
    movieTmdbId: data.movie_results?.[0]?.id ?? null,
    showTmdbId: data.tv_results?.[0]?.id ?? null,
  }
}

async function collectMovieIdsFromPagedEndpoint(path: string, limit: number): Promise<number[]> {
  const apiKey = await getApiKey()
  const ids: number[] = []
  let page = 1

  while (ids.length < limit) {
    const url = `${TMDB_BASE}${path}?api_key=${apiKey}&page=${page}`
    const data = await fetchJson<{ results?: Array<{ id: number }>; total_pages?: number }>(url)
...
export async function getTmdbTopRatedMovieIds(limit = 100): Promise<number[]> {
  return collectMovieIdsFromPagedEndpoint('/movie/top_rated', limit)
}

export async function getTmdbPopularMovieIds(limit = 100): Promise<number[]> {
  return collectMovieIdsFromPagedEndpoint('/movie/popular', limit)
}
